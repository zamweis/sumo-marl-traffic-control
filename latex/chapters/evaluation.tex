...
\begin{enumerate}
  ...
  \item \textbf{Actuated}: SUMO-aktuiert (sofern in \texttt{net.xml} konfiguriert).
\end{enumerate}

\subsection{Simulations- und Umgebungsparameter}
Für alle Simulationen (RL und Baselines) gelten konsistente Parameter:
\begin{itemize}
  \item Episodenlänge: \texttt{EP\_LENGTH\_S = 3000}~s,
...
Das Seed-Design ist zweistufig:
\begin{itemize}
  \item \textbf{Evaluationsgruppen-Seeds} (\texttt{SEEDS=\{1111,2222\}}): Labeln die Auswertung pro Modell–Szenario-Kombination.
  \item \textbf{Episoden-Seeds} (\texttt{EP\_SEEDS}, zehn disjunkte Werte, aktuell fünf genutzt): steuern \emph{pro Episode} den SUMO-Zufallsgenerator und sind vom Training entkoppelt.
\end{itemize}
Pro Kombination von \emph{Modell}, \emph{Reward-Funktion}, \emph{Szenario}, \emph{Seed} und \emph{Methode} (Fixed-Time, Actuated, RL) werden identische Replikationen ausgeführt:
\[
  \#\text{Episoden} = |\texttt{RUNS}| \cdot |\texttt{Rewards}| \cdot 4 \;(\text{Szenarien}) \cdot 2 \;(\text{Seeds}) \cdot 5 \;(\text{Episoden}) \cdot 3 \;(\text{Methoden}).
\]
...
\texttt{evaluation/logs/eval\_\{run\_dir\}\_\{scenario\}\_seed\{seed\}}.
Die Metriken werden nach \texttt{Baseline\_FixedTime/}, \texttt{Baseline\_Actuated/} und \texttt{RL\_<RewardName>/} getrennt protokolliert.
...
\begin{itemize}
  \item aggregierte Mittelwerte der \texttt{infos}-Keys (z.\,B.\ \texttt{mean\_waiting\_time}, \texttt{mean\_speed}, \dots),
  \item \texttt{ep\_rew}, \texttt{ep\_len},
  \item Metadaten: \texttt{scenario}, \texttt{seed}, \texttt{ep\_seed}, \texttt{episode}, \texttt{method} $\in$ \{\texttt{Baseline\_FixedTime}, \texttt{Baseline\_Actuated}, \texttt{RL\_realworld}, \texttt{RL\_pressure}, \texttt{RL\_throughput}, \dots\}, \texttt{run\_dir}.
\end{itemize}
...
Die Struktur erlaubt es, RL-Varianten mit unterschiedlichen Reward-Funktionen direkt gegenüber Fixed-Time und Actuated zu stellen.
...
\end{itemize}
Die deterministische Politik im RL und eingefrorene Normalisierung reduzieren Varianz und erlauben einen klaren Vergleich zwischen allen Methoden (\texttt{RL\_<Reward>}, \texttt{Baseline\_FixedTime}, \texttt{Baseline\_Actuated}).
