...
\begin{itemize}
  ...
  \item \textbf{Visuelle Kontrolle:} Inspektion kritischer Knoten in der \gls{sumogui} und in \texttt{netedit}, um Fehler wie unverbundene Spuren oder falsche Richtungen zu erkennen. \cite{netedit}
  \item \textbf{\gls{tls}-Prüfung:} Alle 17 Lichtsignalanlagen wurden in \texttt{netedit} geöffnet. Phasenpläne, gesteuerte Verbindungen und Zustandslängen (\texttt{state}) wurden geprüft und bei Bedarf korrigiert.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Niedrige Last:} geringes Verkehrsaufkommen mit überwiegend freiem Fluss,
  \item \textbf{Normale Last:} repräsentatives Durchschnittsaufkommen im Tagesverlauf,
  \item \textbf{Hohe Last:} stark verdichteter Verkehr zur Abbildung von Spitzenbelastungen.
\end{itemize}
...
Für das Reinforcement-Learning-Setup wurden die TLS so konfiguriert, dass sie in SUMO als „aktuiert“\footnote{„Aktuiert“ bedeutet in SUMO, dass die Signalanlagen nicht strikt einem festen Schaltplan folgen, sondern dynamisch mit Hilfe von Detektoren oder externen Eingaben reagieren können.}  initialisiert und anschließend direkt durch das RL-Modul über \texttt{TraCI} gesteuert werden konnten. \cite{sumo-rl_docs, sumo-doc}
