...
\begin{itemize}
  \item aktuelle Ampelphase (diskreter Index oder One-Hot-Encoding, dimensionslos),
  \item Dauer der aktuellen Phase in Sekunden (zur Einhaltung von Mindestzeiten),
  \item für jede anliegende Fahrspur:
        \begin{itemize}
          \item Anzahl wartender Fahrzeuge [Fz],
          \item mittlere Geschwindigkeit [m/s],
          \item Fahrdichte (Verhältnis belegter zu maximal möglicher Fahrzeuge pro Spur, dimensionslos, $0\text{–}1$).
        \end{itemize}
\end{itemize}

Da Kreuzungen im Netz unterschiedlich viele anliegende Fahrspuren besitzen, variiert die Rohdimension des Beobachtungsvektors zwischen den Agenten. Um eine einheitliche Eingabelänge für die RL-Policy zu gewährleisten, werden alle Beobachtungsvektoren mittels \texttt{pad\_observations\_v0} (SuperSuit) auf die maximale Spuranzahl aller gesteuerten Kreuzungen gepaddet; nicht vorhandene Spuren werden dabei mit Nullwerten aufgefüllt. Dadurch ergibt sich für alle Agenten ein Beobachtungsvektor gleicher Dimension $(n_{\text{features}} \times n_{\text{max\_spuren}} + n_{\text{phasenfeatures}})$.

Der Aktionsraum ist diskret und erlaubt die Auswahl der nächsten Signalphase. Ein Phasenwechsel wird durch SUMO automatisch mit einer Zwischenphase (Gelbphase) ergänzt. Die Entscheidung erfolgt synchron für alle Agenten in festen Intervallen von \texttt{delta\_time = 5\,s}.
