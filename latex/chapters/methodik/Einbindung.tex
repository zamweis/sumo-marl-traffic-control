...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=0.9cm]
        ...
        \node[box, below=of sim] (mae) {\gls{mae}\\\texttt{sumo-rl.parallel\_env} + \texttt{reward\_fn}};
        ...
        \node[box, below=of wrap] (vec) {\gls{vecnormalize} + \gls{vecmonitor}\\(Stable-Baselines3)};
...
Zusätzlich wurden während des Trainings mit \gls{tensorboard} zentrale Leistungsmetriken (mittlere Geschwindigkeit, Wartezeit, CO$_2$-Emissionen, Durchsatz) in Echtzeit überwacht,
...
Zusätzlich sind mehrere \gls{callback}-Mechanismen implementiert:
\begin{itemize}
    \item \textbf{\gls{checkpoint}:} Zeitbasiertes Speichern der Modelle (stündlich).
    \item \textbf{\gls{logging}:} Aggregierte Episodenmetriken (Wartezeiten, Queues, Fluss) werden in TensorBoard und CSV-Dateien geschrieben.
...
Die Umgebung ist vollständig kompatibel mit \texttt{Gymnasium}, \texttt{PettingZoo} sowie den Wrapper-Bibliotheken \texttt{SuperSuit} und \texttt{\gls{vecenv}}, wodurch ein standardisiertes Interfacing mit RL-Algorithmen ermöglicht wird.
...
Beobachtungen und Rewards wurden über \texttt{VecNormalize} normalisiert (\texttt{clip\_obs = 10.0}, \texttt{clip\_reward = 10.0}, \texttt{norm\_obs = True}, \texttt{norm\_reward = True}), die Umgebung zusätzlich mit \texttt{VecMonitor} überwacht (\texttt{monitor.csv}). Pro Seed wurden $2\cdot10^6$ Trainingsschritte durchgeführt. \cite{VecMonitor, VecNormalize} 
...
Alle relevanten Metriken werden zusätzlich im CSV-Format und über TensorBoard erfasst. \cite{TensorBoard} 
...
\begin{figure}[H]
    ...
    \caption{Beispielhafte Lernkurve eines Trainingslaufs, visualisiert mit TensorBoard. (Quelle: lokales Tensorboard)}
