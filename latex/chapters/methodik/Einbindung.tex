...
Im Gegensatz zu früheren Ansätzen werden \emph{statische Baselines direkt im Evaluationslauf} mitgeführt (fester Phasenplan und aktuiert).

Folgende Dimensionen werden berücksichtigt:
\begin{itemize}
  \item \textbf{Trainierte Modelle:} Es werden mehrere Varianten von RL-Agenten getestet, die sich durch unterschiedliche Belohnungsfunktionen unterscheiden (vgl. Abschnitt~\ref{sec:rl-konzept}).
        Die Evaluationslogik ist so gestaltet, dass beliebig viele Reward-Varianten unter identischen Bedingungen eingebunden werden können.
  \item \textbf{Verkehrsszenarien:} Für die Simulation werden verschiedene Routenprofile genutzt (z.\,B.\ deterministische Flüsse, stochastisch generierte Muster) mit unterschiedlichen Verkehrsstärken (gering, mittel, hoch).
  \item \textbf{Baselines:} Als Referenz dienen SUMO-interne Steuerungen: ein fester Signalplan (Fixed-Time) und eine aktuiert gesteuerte Variante (Actuated).
\end{itemize}
...
Alle Ergebnisse werden über mehrere Seeds und Episoden gemittelt, um die Robustheit der Aussagen zu erhöhen; für die stochastische Variation werden ausschließlich \emph{zuvor ungenutzte} Episoden-Seeds eingesetzt, sodass eine Überanpassung auf spezifische Muster ausgeschlossen wird.

Diese Strategie stellt sicher, dass die in Kapitel~\ref{sec:validation} präsentierten Ergebnisse reproduzierbar, belastbar und sowohl zwischen RL-Varianten als auch im Vergleich zu Baselines aussagekräftig sind.
