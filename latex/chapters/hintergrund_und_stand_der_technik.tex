...
\textit{Simulation of Urban MObility} (SUMO) ist ein quelloffener,  Verkehrs-Simulator, der ursprünglich vom Deutschen Zentrum für Luft- und Raumfahrt (DLR) entwickelt wurde. \cite{sumo-doc} SUMO erlaubt die detaillierte Modellierung individueller Fahrzeuge, Straßeninfrastruktur, Ampelschaltungen sowie Fahrverhalten. \cite{sumo-doc, Sumo-git, Sumo-Publikation}

Besonders relevant für diese Arbeit sind folgende Merkmale\cite{sumo-doc}:
...
\begin{itemize}
    ...
    \item \textbf{Flexible Netzdefinition:} Verkehrsnetze lassen sich aus OpenStreetMap-Daten sowie aus Shapefiles oder VISUM-Modellen mit dem Tool \texttt{netconvert} erzeugen. \cite{netconvert} Netzdateien können auch mit \texttt{netedit} visuell editiert werden.
    \item \textbf{Nachfragegenerierung:} Fahrpläne und Routen lassen sich mit Tools wie \texttt{activitygen}, \texttt{randomTrips}, \texttt{od2trips} oder \texttt{duarouter} erzeugen, basierend auf statistischen oder echten OD-Matrizen. \cite{randomTrips,duarouter}
    ...
    \item \textbf{Emissionsmodellierung:} Mit Hilfe von integrierten HBEFA-Tabellen (Version 4)\cite{Sumo-HBEFA} kann SUMO CO\textsubscript{2}-, NO\textsubscript{x}- und Feinstaubemissionen simulieren und ausgeben.
...
Ein zentrales Element für externe Steuerungsexperimente ist das
\textbf{Traffic Control Interface (TraCI)\cite{sumo-doc}}.
Dabei handelt es sich um eine TCP-basierte Client-Server-Schnittstelle,
...
auf den Verkehrsfluss auswirken können und unmittelbares Feedback erhalten. \cite{TraCI}
\label{sec:TraCI}
...
Reinforcement Learning (RL) ist ein Teilgebiet des maschinellen Lernens, bei dem ein Agent durch Interaktion mit einer Umgebung lernt, optimale Handlungen auszuführen. Ziel ist es, die kumulative Belohnung über Zeit zu maximieren \cite{sutton-barto}.
...
\begin{itemize}
    \item \textbf{phase\_one\_hot}: One-Hot-Kodierung der aktuell aktiven Grünphase.
...
Der Aktionsraum ist diskret. Agents dürfen alle \texttt{delta\_time} Sekunden eine neue Grünphase wählen. Jeder Aktionswert repräsentiert dabei eine bestimmte, zulässige Grünkonfiguration für die Kreuzung. Ein Phasenwechsel wird stets durch eine automatisch hinzugefügte Gelbphase (\texttt{yellow\_time} Sekunden) eingeleitet.
...
\begin{itemize}
    ...
    \item \textbf{Multi-Agent-Fähigkeit}: Unterstützt Einzel- und Mehragentenumgebungen; bei Verwendung von \texttt{parallel\_env} über PettingZoo werden alle Agenten synchron gesteuert.
    \item \textbf{Systemmetriken (Infos)}: Die Umgebung kann global relevante Kennzahlen wie Wartezeit, Gesamtemissionen, Backlog etc. über das \texttt{infos}-Objekt bereitstellen – steuerbar über Parameter wie \texttt{add\_system\_info} oder \texttt{add\_per\_agent\_info}.
    \item \textbf{Kompatibilität mit RL-Bibliotheken}: Nativ kompatibel mit Stable-Baselines3, PyTorch, TensorFlow, RLlib etc., was eine einfache Einbettung in bestehende RL-Pipelines erlaubt.
...
\begin{itemize}
    ...
    \item \textbf{Zheng et al. (2022):} Vorschlag eines Curriculum-Learning-Ansatzes, der schrittweise von lokalen zu globalen Steuerstrategien führt,mit dem Ziel, RL-Agenten robuster und lernfähiger zu machen. \cite{zheng2022}
