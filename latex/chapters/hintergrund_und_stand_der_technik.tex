...
\textit{Simulation of Urban Mobility} ist ein quelloffener,  Verkehrs-Simulator, der ursprünglich vom \gls{dlr} entwickelt wurde. \cite{sumo-doc} SUMO erlaubt die detaillierte Modellierung individueller Fahrzeuge, Straßeninfrastruktur, Ampelschaltungen sowie Fahrverhalten. \cite{sumo-doc, Sumo-git, Sumo-Publikation}
...
\begin{itemize}
    ...
    \item \textbf{Flexible Netzdefinition:} Verkehrsnetze lassen sich aus OpenStreetMap-Daten sowie aus \gls{shapefiles} oder \gls{visum}-Modellen mit dem Tool \gls{netconvert} erzeugen. \cite{netconvert} Netzdateien können auch mit \texttt{netedit} visuell editiert werden.
    ...
    \item \textbf{Emissionsmodellierung:} Mit Hilfe von integrierten \gls{hbefa}-Tabellen (Version 4)\cite{Sumo-HBEFA} kann SUMO CO\textsubscript{2}-, NO\textsubscript{x}- und Feinstaubemissionen simulieren und ausgeben.
...
Ein zentrales Element für externe Steuerungsexperimente ist das TraCI\cite{sumo-doc}.
...
In dieser Arbeit wird ausschließlich \textbf{\gls{ppo}} \cite{Proximal_Policy_Optimization} eingesetzt, ein policy-basierter RL-Algorithmus, der für seine Stabilität und Effizienz bei kontinuierlichen Interaktionen mit komplexen Umgebungen bekannt ist. Andere RL-Verfahren (z. B. \gls{qlearning} oder \gls{dqn}) wurden nicht berücksichtigt.
