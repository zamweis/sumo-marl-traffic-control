...
Reinforcement Learning (RL) ist ein Teilgebiet des maschinellen Lernens, bei dem ein Agent durch Interaktion mit einer Umgebung lernt, optimale Handlungen auszuführen. Dabei verfolgt er das Ziel, eine kumulative Belohnung zu maximieren. \cite{sutton-barto}
...
\begin{itemize}
    \item \textbf{Q-Learning\cite{Q-Learning}:} Modellfreies, off-policy Lernverfahren zur Annäherung an optimale Aktionen.
    \item \textbf{DQN (Deep Q-Network)\cite{Deep_Q-Learning}:} Kombination von Q-Learning mit neuronalen Netzen.
    \item \textbf{PPO (Proximal Policy Optimization)\cite{Proximal_Policy_Optimization}:} Policy-basierter RL-Ansatz mit stabiler Optimierung.
\end{itemize}

\subsection{SUMO-RL: Architektur und Funktionalität}

\texttt{sumo-rl} ist ein Python-Framework, das SUMO mit Reinforcement Learning verbindet. Es basiert auf der \texttt{gymnasium}-Schnittstelle\cite{Gymnasium} und abstrahiert typische Aufgaben wie die Definition von Beobachtungen, Aktionen und Belohnungen für RL-Agenten. Die Umgebung wird durch die Klasse \texttt{SumoEnvironment} bereitgestellt.
...
\begin{itemize}
    ...
    \item \textbf{Ein- und Mehragentenunterstützung:} \texttt{sumo-rl} unterstützt sowohl Single-Agent-Setups als auch Multi-Agent-Steuerung über die PettingZoo-API\cite{PettingZoo}. Jeder gesteuerte Knoten im Netz kann einem eigenen Agenten zugewiesen werden.
    ...
    \item \textbf{Kompatibilität:} Das Framework ist kompatibel mit Stable-Baselines3\cite{Stable-Baselines3}, PyTorch\cite{pytorch}, TensorFlow\cite{TensorFlow}, RLlib\cite{RLlib} und anderen gängigen ML-Frameworks.
\end{itemize}
