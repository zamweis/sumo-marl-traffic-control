...
# ====== Globaler Cache ======
_last_step_cache = {"step": None, "mean_emis": 0.0, "arrived": 0}

# ====== Trainings-Setup ======
SEEDS = [546456, 678678, 234256, 678]  # Verschiedene Zufalls-Seed-Werte für reproduzierbare Runs

# ====== Reward-Funktion: Emission-Variante ======
def flow_emissions_reward(ts):
    current_step = int(traci.simulation.getTime())

    # Cache, damit pro Step nur einmal messen
    if _last_step_cache["step"] != current_step:
        # Fahrzeuge im Netz
        vids = traci.vehicle.getIDList()
        n_veh = len(vids)

        # Durchschnittliche Emission pro Fahrzeug
        if n_veh > 0:
            total_emis = sum(traci.vehicle.getCO2Emission(vid) for vid in vids)
            mean_emissions = total_emis / n_veh
        else:
            mean_emissions = 0.0

        # Metriken aus SUMO / TS-Objekt
        arrived_now = traci.simulation.getArrivedNumber()
        queue_len = ts.get_total_queued()

        # Im Cache speichern
        _last_step_cache.update({
            "mean_emis": mean_emissions,
            "arrived": arrived_now,
            "queue": queue_len,
            "step": current_step
        })

    # Normierung (typische Max-Werte anpassen)
    norm_queue = _last_step_cache["queue"] / 30.0         # 30 Fahrzeuge Stau = 1.0
    norm_flow = _last_step_cache["arrived"] / 10.0        # 10 ankommende Fahrzeuge = 1.0
    norm_emissions = _last_step_cache["mean_emis"] / 500.0  # 500 mg/s ≈ 1.0

    # Gewichtung (Feintuning je nach Ziel)
    w_queue = 0.4
    w_flow = 0.5
    w_emis = 0.3

    # Reward: Weniger Stau, mehr Durchfluss, weniger Emissionen
    reward = (
        -w_queue * np.tanh(norm_queue) +
         w_flow  * np.tanh(norm_flow) -
         w_emis  * np.tanh(norm_emissions)
    )
...
for SEED in SEEDS:
    ...
    env = parallel_env(
        ...
        reward_fn=flow_emissions_reward,  # Reward-Funktion
